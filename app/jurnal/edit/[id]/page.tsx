...
export default function EditJournalPage() {
  ...
  useEffect(() => {
    async function loadJournalData() {
      try {
        ...
        // 2. Load data jurnal DAN cek ownership
        const { data: journal, error } = await supabase
          .from('alumni_journals')
          .select('*')
          .eq('id', journalId)
          .eq('author_id', user.id) // PENTING: Filter by author_id
          .single()
        
        if (error || !journal) {
          // Jurnal tidak ditemukan atau bukan milik user
          setError("Jurnal tidak ditemukan atau Anda tidak memiliki akses untuk mengeditnya")
          setTimeout(() => router.push('/jurnal'), 2000)
          return
        }
        ...
        setCreatedByName(journal.created_by_name || "")
        
        if (journal.cover_image_url) {
          setCoverImageUrl(journal.cover_image_url)
          setPreview(journal.cover_image_url)
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update jurnal - RLS policy akan otomatis cek ownership
      const { error } = await supabase
        ...
        .eq('id', journalId)
        .eq('author_id', userId) // Double check
      ...
      setError(error instanceof Error ? error.message : "Terjadi kesalahan saat mengupdate jurnal")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground">Memuat data jurnal...</p>
        </div>
      </div>
    )
  }

  // Error state (not found or no access)
  if (error && !userId) {
    return (
      <div className="container mx-auto px-4 py-12">
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="max-w-3xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">Edit Jurnal</CardTitle>
            <CardDescription>Perbarui jurnal kamu</CardDescription>
          </CardHeader>
          <CardContent>
